...
export function generateInvoicePdf({ invoice, labName, labAddress, labTaxNumber, t }: InvoicePdfData) {
  ...
  bold(ctx, 16);
  doc.text(t.invoices.title.toUpperCase(), ctx.rightCol, ctx.y + 2, { align: 'right' });
  ...
  if (labAddress) {
    normal(ctx, 8);
    doc.text(labAddress, margin, ctx.y);
    ctx.y += 3.5;
  }
  if (labTaxNumber) {
    normal(ctx, 8);
    doc.text(`${t.doctors.taxNumber}: ${labTaxNumber}`, margin, ctx.y);
    ctx.y += 3.5;
  }

  // Invoice number
  bold(ctx, 10);
  doc.text(invoice.invoiceNumber, ctx.rightCol, ctx.y - 2, { align: 'right' });
  ...
  bold(ctx, 9);
  doc.text(t.invoices.doctorPartner.toUpperCase(), margin, ctx.y);
  ctx.y += 5;

  normal(ctx, 9);
  if (invoice.doctorPartner) {
    doc.text(invoice.doctorPartner.doctorName, margin, ctx.y);
    if (invoice.doctorPartner.clinicName) {
      ctx.y += 4;
      doc.text(invoice.doctorPartner.clinicName, margin, ctx.y);
    }
    if (invoice.doctorPartner.address) {
      ctx.y += 4;
      doc.text(invoice.doctorPartner.address, margin, ctx.y);
    }
    if (invoice.doctorPartner.taxNumber) {
      ctx.y += 4;
      doc.text(`${t.doctors.taxNumber}: ${invoice.doctorPartner.taxNumber}`, margin, ctx.y);
    }
  }
  ...
  bold(ctx, 9);
  doc.text(`${t.invoices.issueDate}:`, midX, dateY);
  normal(ctx, 9);
  doc.text(formatPdfDate(invoice.issueDate), midX + 35, dateY);

  if (invoice.dueDate) {
    bold(ctx, 9);
    doc.text(`${t.invoices.dueDate}:`, midX, dateY + 5);
    normal(ctx, 9);
  ...
  if (invoice.paidAt) {
    bold(ctx, 9);
    doc.text(`${t.invoices.paidAt}:`, midX, dateY + 10);
    normal(ctx, 9);
  ...
  bold(ctx, 9);
  doc.text(t.invoices.items.toUpperCase(), margin, ctx.y);
  ctx.y += 6;
  ...
  doc.text(t.invoices.description, cols.desc, ctx.y);
  doc.text(t.invoices.quantity, cols.qty, ctx.y);
  doc.text(t.invoices.unitPrice, cols.unit, ctx.y);
  doc.text(t.invoices.lineTotal, cols.total, ctx.y, { align: 'right' });
  ctx.y += 5;
  ...
  bold(ctx, 11);
  doc.text(`${t.invoices.totalAmount}:`, cols.unit - 22, ctx.y);
  doc.text(formatPdfCurrency(invoice.totalAmount, invoice.currency), cols.total, ctx.y, { align: 'right' });
  ctx.y += 8;
  ...
  if (invoice.notes) {
    ...
    ctx.y += 2;
    bold(ctx, 9);
    doc.text(t.invoices.notes.toUpperCase(), margin, ctx.y);
    ctx.y += 5;
    normal(ctx, 8);
    const noteLines = doc.splitTextToSize(invoice.notes, ctx.contentWidth);
  ...
  normal(ctx, 8);
  doc.text(labName || 'Labor', sigLeftX + sigLineWidth / 2, ctx.y, { align: 'center' });
  doc.text(invoice.doctorPartner?.doctorName || '', sigRightX + sigLineWidth / 2, ctx.y, { align: 'center' });
